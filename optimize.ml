...
let rec fold_constant (e: expr) (env: env) : env * expr = 
  ...
  | Seq es -> let _, e_list = compile_seq env es in
              return (Seq e_list)
  | Unary (op, e') -> let new_e = recur e' in
                      (match new_e with
                       | Const(CBool(v)) -> return (Const(CBool(not v)))
                       | _ -> return (Unary (op, (recur new_e)))
                       )
  | Binary (op, e1, e2) -> return (compile_binary env recur op e1 e2 e)
  | Ite (ec, et, ef) -> let new_ec = recur ec in
                        (match new_ec with
                        | Const(CBool(true)) -> return (recur et)
                        | Const(CBool(false)) -> return (recur ef)
                        | _ -> return (Ite (recur ec, recur et, recur ef))
                        )
